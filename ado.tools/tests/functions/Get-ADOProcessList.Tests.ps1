Describe "Get-ADOProcessList Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ADOProcessList).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Get-ADOProcessList).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-ADOProcessList).Parameters['Token']
		...
		It 'Should have the expected parameter Expand' {
			$parameter = (Get-Command Get-ADOProcessList).Parameters['Expand']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Get-ADOProcessList).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-ADOProcessList).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -Organization -Token -Expand -ApiVersion -ProgressAction
		#>
	}
