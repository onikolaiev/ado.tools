Describe "Get-ADOProcess Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ADOProcess).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Get-ADOProcess).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-ADOProcess).Parameters['Token']
		...
		It 'Should have the expected parameter ProcessTypeId' {
			$parameter = (Get-Command Get-ADOProcess).Parameters['ProcessTypeId']
		...
		It 'Should have the expected parameter Expand' {
			$parameter = (Get-Command Get-ADOProcess).Parameters['Expand']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Get-ADOProcess).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-ADOProcess).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -Token -ProcessTypeId
		__AllParameterSets -Organization -Token -ProcessTypeId -Expand -ApiVersion -ProgressAction
		#>
	}
