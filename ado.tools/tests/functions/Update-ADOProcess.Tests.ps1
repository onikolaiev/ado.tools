Describe "Update-ADOProcess Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Update-ADOProcess).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Update-ADOProcess).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Update-ADOProcess).Parameters['Token']
		...
		It 'Should have the expected parameter ProcessTypeId' {
			$parameter = (Get-Command Update-ADOProcess).Parameters['ProcessTypeId']
			$parameter.Name | Should -Be 'ProcessTypeId'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter Body' {
			$parameter = (Get-Command Update-ADOProcess).Parameters['Body']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Update-ADOProcess).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Update-ADOProcess).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -Token -ProcessTypeId -Body
		__AllParameterSets -Organization -Token -ProcessTypeId -Body -ApiVersion -ProgressAction
		#>
	}
