Describe "Remove-ADOProject Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Remove-ADOProject).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Remove-ADOProject).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Remove-ADOProject).Parameters['Token']
		...
		It 'Should have the expected parameter ProjectId' {
			$parameter = (Get-Command Remove-ADOProject).Parameters['ProjectId']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Remove-ADOProject).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Remove-ADOProject).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -Organization -Token -ProjectId -ApiVersion -ProgressAction
		#>
	}
