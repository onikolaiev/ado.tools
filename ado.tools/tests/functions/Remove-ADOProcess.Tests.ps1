Describe "Remove-ADOProcess Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Remove-ADOProcess).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Remove-ADOProcess).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Remove-ADOProcess).Parameters['Token']
		...
		It 'Should have the expected parameter ProcessTypeId' {
			$parameter = (Get-Command Remove-ADOProcess).Parameters['ProcessTypeId']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Remove-ADOProcess).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Remove-ADOProcess).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -Organization -Token -ProcessTypeId -ApiVersion -ProgressAction
		#>
	}
