Describe "Get-ADOSettings Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ADOSettings).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter SettingsJsonString' {
			$parameter = (Get-Command Get-ADOSettings).Parameters['SettingsJsonString']
		...
		It 'Should have the expected parameter SettingsJsonPath' {
			$parameter = (Get-Command Get-ADOSettings).Parameters['SettingsJsonPath']
		...
		It 'Should have the expected parameter OutputAsHashtable' {
			$parameter = (Get-Command Get-ADOSettings).Parameters['OutputAsHashtable']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-ADOSettings).Parameters['ProgressAction']
