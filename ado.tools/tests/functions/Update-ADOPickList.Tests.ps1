Describe "Update-ADOPickList Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Update-ADOPickList).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Update-ADOPickList).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Update-ADOPickList).Parameters['Token']
		...
		It 'Should have the expected parameter ListId' {
			$parameter = (Get-Command Update-ADOPickList).Parameters['ListId']
			$parameter.Name | Should -Be 'ListId'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter Body' {
			$parameter = (Get-Command Update-ADOPickList).Parameters['Body']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Update-ADOPickList).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Update-ADOPickList).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -Token -ListId -Body
		__AllParameterSets -Organization -Token -ListId -Body -ApiVersion -ProgressAction
		#>
	}
