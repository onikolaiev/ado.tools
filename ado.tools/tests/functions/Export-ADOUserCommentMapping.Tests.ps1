Describe "Export-ADOUserCommentMapping Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Export-ADOUserCommentMapping).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['Organization']
		...
		It 'Should have the expected parameter ProjectName' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['ProjectName']
			$parameter.Name | Should -Be 'ProjectName'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['Token']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter OutputPath' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['OutputPath']
		...
		It 'Should have the expected parameter Force' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['Force']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Export-ADOUserCommentMapping).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -ProjectName -Token
		__AllParameterSets -Organization -ProjectName -Token -ApiVersion -OutputPath -Force -ProgressAction
		#>
	}
