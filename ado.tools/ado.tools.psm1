...
$script:ModuleVersion = (Import-PowerShellDataFile -Path "$($script:ModuleRoot)\ado.tools.psd1").ModuleVersion

# Detect whether at some level dotsourcing was enforced
$script:doDotSource = Get-PSFConfigValue -FullName ado.tools.Import.DoDotSource -Fallback $false
if ($ado.tools_dotsourcemodule) { $script:doDotSource = $true }
...
$importIndividualFiles = Get-PSFConfigValue -FullName ado.tools.Import.IndividualFiles -Fallback $false
if ($ado.tools_importIndividualFiles) { $importIndividualFiles = $true }
